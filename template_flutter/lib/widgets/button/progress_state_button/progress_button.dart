...
class ProgressButton extends StatefulWidget {
  ...
  factory ProgressButton.icon({
    required Map<ButtonState, StateButton> stateButton,
    Function? onPressed,
    ...
    assert(
    stateButton != null &&
        stateButton.keys.toSet().containsAll(ButtonState.values.toSet()),
    'Must be non-null widgets provided in map of stateWidgets. Missing keys => ${ButtonState.values.toSet().difference(stateButton.keys.toSet())}',
    );
    ...
    Map<ButtonState, Widget> stateWidgets = {
      ButtonState.idle: buildChildWithIcon(
          stateButton[ButtonState.idle]!, iconPadding, textStyle),
      ButtonState.loading: Column(),
      ButtonState.fail: buildChildWithIcon(
          stateButton[ButtonState.fail]!, iconPadding, textStyle),
      ButtonState.success: buildChildWithIcon(
          stateButton[ButtonState.success]!, iconPadding, textStyle)
    };

    Map<ButtonState, Color> stateColors = {
      ButtonState.idle: stateButton[ButtonState.idle]!.color,
      ButtonState.loading: stateButton[ButtonState.loading]!.color,
      ButtonState.fail: stateButton[ButtonState.fail]!.color,
      ButtonState.success: stateButton[ButtonState.success]!.color,
    };
